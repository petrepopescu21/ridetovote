...
const inputFile = process.argv[2] || 'sectii_votare.csv';
const idsFile = process.argv[3] || '../registered.txt';
const outputFile = process.argv[4] || 'filtered.json';
...
async function processCSV() {
  try {
    ...
    console.log(`Records to process: ${recordsToProcess.length} of ${idsToProcess.size} requested IDs`);
    ...
    if (notFoundIds.length > 0) {
      console.log(`Warning: ${notFoundIds.length} IDs were not found in the CSV: ${notFoundIds.join(', ')}`);
    }
    ...
    for (let i = 0; i < recordsToProcess.length; i++) {
      ...
      const id = record['Nr SV'];
      const address = record['Adresa SV'];
